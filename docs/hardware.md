...
> **Основное питание:** Arduino Nano получает питание по USB (+5V) от головного устройства. ГУ обеспечивает стабилизированное, отфильтрованное питание. Изолированная шина +3.3V_Isolated формируется из USB-питания Arduino (через встроенный LDO 3.3V или внешний источник — уточняется по схеме).

```mermaid
flowchart LR
    USB[USB +5V от ГУ] -->|Arduino Nano| U77[+5V_Arduino]
    U77 -->|I2C, GPIO| ISO[ISO1540DR / оптопары]
    VIN_iso[+3.3V_Isolated] --> Q1_S[Q1 Исток<br/>SOP-8 пин 1-3]
    Q1_D[Q1 Сток<br/>SOP-8 пин 5-8] --> ACC_3.3V_K[Шина ACC_3.3V_K]
    Q1_G[Q1 Затвор<br/>SOP-8 пин 4] --> GND_K[GND_K]
    ACC_3.3V_K --> MCP[Питание MCP4561, подтяжки I2C]
```

MOSFET Q1 (HIRF7424TRPBF, корпус SOP-8) выполняет функцию **коммутации изолированной шины +3.3V_Isolated**, а не защиты от выбросов бортсети (защита не требуется — питание по USB от ГУ).
...
| `Q1_1` | `ACC_3.3V_K` | Q1-1,-2,-3 (исток), CN1-1 |
...
|---|---|---|
| 1 | `+3.3V_Isolated` (вход) | Внешнее питание изолированной стороны (+3.3V), соединён с Q1 (исток, пины 1-3). Источник: может подаваться от внешнего стабилизированного источника либо формироваться из USB +5V Arduino. Требует уточнения по схеме. |
...
| 4 | `ACC_3.3V_K` (выход) | Коммутируемое через U33 питание +3.3V_Isolated для внешнего устройства. Подаётся как цифровой логический сигнал (импульс 500 мс при смене пресета). |
